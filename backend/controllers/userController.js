...
export const registerUser = async (req, res) => {
  ...
  try {
    ...
    const tokenExpiresAt = Date.now() + 1000 * 60 * 60 * 24; // 1 day
    ...
    const emailResponse = await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f9f9f9; border: 1px solid #ddd; border-radius: 8px;">
          ...
          <div style="text-align: center; margin: 20px 0;">
            <a href="${resetLink}" style="background-color:#4B89FF; color: white; padding: 12px 24px; text-decoration: none; font-size: 16px; border-radius: 4px; display: inline-block;">
              Reset Password
            </a>
          </div>
